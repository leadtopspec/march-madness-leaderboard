          ...
          <div className="md:hidden">
            <div className="flex items-center justify-between py-4">
              <div className="flex items-center space-x-3">
                <div className={`w-12 h-12 rounded-xl bg-gradient-to-r ${agentBadge.color} flex items-center justify-center shadow-lg relative`}>
                  <AgentIcon className="w-6 h-6 text-white" />
                  <div className="absolute -top-1 -right-1 w-6 h-6 bg-slate-800 text-white rounded-full flex items-center justify-center text-xs font-bold">
                    #{agent.rank}
                  </div>
                </div>
                <div>
                  ...
                  <p className="text-xs text-red-300 font-semibold">March Madness</p>
                </div>
              </div>
              ...
              <div className="flex items-center space-x-3">
                ...
                <button
                  onClick={onLogout}
                  className="bg-gradient-to-r from-red-500 to-pink-500 text-white px-3 py-2 rounded-lg"
                >
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        <div className="space-y-6 lg:space-y-0 lg:grid lg:grid-cols-3 lg:gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <motion.div 
              className={`${agentBadge.bg} border-2 rounded-2xl p-6 ${agentBadge.glow} shadow-2xl`}
            ...
            >
              ...
              <div className="mb-6">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="text-right bg-black/30 rounded-lg p-2">
                    <div className="text-sm text-gray-400">Rank</div>
                    <div className="text-xl font-black text-white">#{agent.rank}</div>
                  </div>
                </div>
                <p className="text-red-300 font-semibold">{agent.team}</p>
              </div>
            ...
            {/* Current Matchup - Mobile First */}
            <motion.div 
              className="bg-gradient-to-br from-red-900/40 via-black/60 to-gray-900/40 backdrop-blur-xl rounded-2xl shadow-2xl border-2 border-red-600 p-6"
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              ...
              {currentMatchup?.opponent ? (
                <div className="text-center">
                  ...
                  <div className="space-y-4 mb-6">
                    {/* You */}
                    <div className="bg-red-600/30 border-2 border-red-400 rounded-xl p-4">
                    ...
                    <div className="text-center py-2">
                      ...
                      <div className="text-sm text-gray-400">Win to advance</div>
                    </div>

                    {/* Opponent */}
                    <div className="bg-gray-700/30 border-2 border-gray-500 rounded-xl p-4">
                      <div className="flex items-center justify-between">
                        <div className="text-left">
                      ...
                      <div className="mt-2 text-center">
                        <div className="text-lg font-bold text-green-400">{formatCurrency(currentMatchup.opponent.totalPremium).replace('$', '').replace(',000', 'K')}</div>
                      </div>
                    </div>
                  </div>
                ...
                <div className="text-center py-8">
                  ...
                  <div className="text-2xl font-black text-gray-400 mb-2">Matchup Loading...</div>
                  <div className="text-base text-gray-500 mb-4">Checking your tournament position...</div>
                </div>
              )}

            </motion.div>

            {/* My Recent Sales - Mobile First */}
            <motion.div 
              className="bg-black/90 backdrop-blur-xl rounded-2xl shadow-2xl border-2 border-red-600 p-6"
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              <div className="flex items-center space-x-3 mb-6">
                ...
                <h3 className="text-xl font-black text-white">🌊 My Recent Sales</h3>
              </div>
                ...
                <AnimatePresence>
                  {mySales.length === 0 ? (
                    <div className="text-center py-8">
                      ...
                      <div className="text-xl font-bold text-gray-400 mb-2">No sales recorded yet</div>
                      <div className="text-base text-gray-500">Start riding the wave!</div>
                    </div>
                  ) : (
                    mySales.map((sale, index) => (
                      <motion.div
                        ...
                        className="bg-gradient-to-r from-red-900/20 to-red-800/20 border-2 border-red-600 rounded-xl p-4 backdrop-blur-sm"
                      >
                        <div className="flex justify-between items-start">
                          <div className="flex-1 min-w-0">
                            <div className="font-black text-white text-base truncate">{sale.clientName}</div>
                            <div className="text-sm text-red-300 font-semibold">{sale.policyType}</div>
                            <div className="text-xs text-gray-400">{sale.timestamp.toLocaleDateString()}</div>
                          </div>
                          <div className="flex items-start space-x-2 flex-shrink-0">
                            <div className="text-right">
                              <div className="font-black text-green-400 text-lg">{formatCurrency(sale.premium)}</div>
                            </div>
                            <button
                              onClick={() => {
          ...
          {/* Record Sale Sidebar - Mobile First */}
          <div className="space-y-6">
            ...
            {/* Tournament Position */}
            <motion.div 
              className="bg-gradient-to-br from-red-900/40 to-black/60 rounded-2xl shadow-2xl border-2 border-red-600 p-6 backdrop-blur-sm"
              initial={{ opacity: 0, x: 20 }}
              animate={{ opacity: 1, x: 0 }}
              transition={{ delay: 0.1 }}
            >
              <div className="flex items-center space-x-3 mb-4">
                ...
                <h3 className="text-base font-black text-white">🏖️ Your Position</h3>
              </div>
              
              <div className="space-y-3">
                <div className="text-center bg-red-900/80 backdrop-blur-sm rounded-lg p-4 border border-red-600">
                  <div className="text-3xl font-black text-red-300">#{agent.rank}</div>
                  <div className="text-sm font-bold text-white">Current Rank</div>
                </div>
                
                {agent.rank <= 8 && (
                  <div className="text-center bg-green-900/80 backdrop-blur-sm rounded-lg p-4 border border-green-600">
                    <div className="text-base font-black text-green-300">🏝️ IN THE BRACKET!</div>
                    <div className="text-xs text-green-400">You&apos;re competing for the championship</div>
                  </div>
                )}
                
                {agent.rank > 8 && (
                  <div className="text-center bg-orange-900/80 backdrop-blur-sm rounded-lg p-4 border border-orange-600">
                    <div className="text-base font-black text-orange-300">⚡ PUSH TO TOP 8!</div>
            ...
            >
              <div className="flex items-center space-x-3 mb-4">
                ...
                <h3 className="text-base font-black text-white">⚡ Quick Facts</h3>
              </div>
